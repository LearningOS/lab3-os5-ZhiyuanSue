...
use crate::mm::{translated_refmut, translated_str,get_ptr_physical_addr};
use crate::task::{
    add_task, current_task, current_user_token, exit_current_and_run_next,
    suspend_current_and_run_next, TaskStatus,
    task_map_an_area,task_unmap_an_area,get_current_task,
    TaskControlBlock,
};
...
pub fn sys_task_info(ti: *mut TaskInfo) -> isize {
    ...
    unsafe {
        ...
        (*ti).syscall_times=task_info.task_syscall_times;
        (*ti).time=curr_time/1000-task_info.task_start_time/1000;
    }
    0
}
...
pub fn sys_spawn(_path: *const u8) -> isize {
    ...
    if let Some(data) = get_app_data_by_name(path.as_str()) {
        let curr_task = current_task().unwrap();
        let new_task = curr_task.spawn(data);
        let new_pid = new_task.pid.0;
        add_task(new_task);
        new_pid as isize
    } else {
